...
const imgBunny = new URL('../assets/bunny.png', import.meta.url);
const sndJump = new URL('../assets/jump.wav', import.meta.url);
...
const bootstrap = async () => {
	...
	const CAction = defineComponent({ action: Types.ui16 });

	/* WORLD & CONFIG */
	const world: IWorld & IWorldTime & IWorldRenderer2d & IWorldAudio = createWorld();
	...
	/* DATA  */
	await createSource(world, sndJump.toString());
	await createSprite(world, imgBunny.toString());
	for (let i = 0; i <= 100; i++) {
		...
		CSprite.spriteId[entityId] = getSpriteId(world, imgBunny.toString());
	}
	...
	const SMovement = <T extends IWorld & IWorldTime>(world: T): T => {
		...
		for (const eid of QMovement(world)) {
	...
	const SGravity = <T extends IWorld & IWorldTime>(world: T): T => {
		const { time: { updateDelta: dt } } = world;
		for (const eid of QGravity(world)) {
			const velocity = CVelocity2d.vec2[eid];
			velocity[1] += 100 * dt;
		}
		return world
	...
	const SActionToSource = <T extends IWorld & IWorldAudio>(world: T): T => {
		for (const eid of QActionEnter(world)) {
			if (CAction.action[eid] === ACTIONS.JUMP) {
				...
				CSource.sourceId[newEid] = getSourceId(world, sndJump.toString());
			}
		}
		return world;
	}
